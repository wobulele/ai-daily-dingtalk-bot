...
每天北京时间 `09:00`、`10:00`、`11:00`、`12:00` 读取 [AI 资讯日报 RSS](https://justlovemaki.github.io/CloudFlare-AI-Insight-Daily/rss.xml)，如果出现新的日报条目，则将 `标题 + 摘要 + 链接` 推送到钉钉群机器人。
...
- 定时：每天北京时间 `09:00`、`10:00`、`11:00`、`12:00`
- 换算成 UTC cron：`0 1,2,3,4 * * *`
- 也支持手动 `workflow_dispatch`

由于项目会在 `data/state.json` 中记录最近一次成功推送的日报 `guid/link`，因此即使一天内检查 4 次，也只会在发现当天新日报的第一次发送成功时推送一次，后续同一天检查会自动跳过。
